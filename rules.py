    ...
    for i in prefixes:
        if forms[num].startswith(i):
            ...
            if i != 'с' and ((i[-1] == 'с' and forms[num + 1][0] in 'аеёиоуыэюябвгджзйлмнр') or (
                ...
                continue
            if forms[num + 1] in words or (forms[num + 1].startswith('ы') and ('и' + forms[num + 1][1:] in words)):
                new_flag = True
    ...
    if mark:
        ...
        if not mark:
            if p.tag.POS == 'NOUN' and 'Fixd' not in p.tag:
                ...
                if 'ва' not in word_parts and unknown not in {'кова', 'кива'}:
                ...
                if 'ва' not in word_parts and 'ка' not in word_parts and 'нича' not in word_parts and 'ича' not in word_parts and 'а' not in word_parts:
                    for x in {'и', 'а', 'я', 'е'}:
                        word_parts, labels = identify_suffix_simple(x)
